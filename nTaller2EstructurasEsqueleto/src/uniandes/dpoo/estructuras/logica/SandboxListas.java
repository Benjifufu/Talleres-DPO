...
public class SandboxListas
{
    ...
    public void reiniciarArregloCadenas( List<Object> objetos )
    {
        ...
        // Recorre la lista de objetos y convierte cada objeto a su representación en cadena
        for (Object objeto : objetos) {
            // Agrega la representación en cadena del objeto a la lista
    ...
    public int contarEnterosRepetidos() {
        ...
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            // Si el número no ha sido marcado como repetido
            if (actual != Integer.MIN_VALUE) {
                for (int j = i + 1; j < listaEnteros.size(); j++) {
                    if (actual == listaEnteros.get(j)) {
                        contador++; // Cuenta como repetido
                        listaEnteros.set(j, Integer.MIN_VALUE); // Marca el número como contado
                        break; // Ya no necesitas seguir buscando en la lista
                    }
                }
            }
        }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
        ...
        for (int i = 0; i < cantidad; i++) {
            int numero = rand.nextInt((maximo - minimo + 1)) + minimo;
